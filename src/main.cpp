...
#if defined(NDEBUG)
# error "Iskies cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x084afbcc66fd04a0e961881801a79d763044cc7c587e3d8415ee06602eefae9c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Iskies: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Iskies Signed Message:\n";
...
{
    // Iskies: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Iskies: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Iskies: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Iskies: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Iskies: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Iskies: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbacf256d9b84f66eeb2e8f2b97b7383f620a007093e2064a973a595a0b332676");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Its Official! Spice Girls confirm REUNION 02/02/18";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a4c970bfe94c4d00c9b9bd75debec3d983b29de80d81ea77c0005863b02e5d03f4d5d1bbda86a0242600152274e7fa1a2ff4cf29e8818fc836b022bac8ffeb51") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517611968;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085421850;

        if (fTestNet)
        {
            block.nTime    = 1517611895;
            block.nNonce   = 385592837;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xbe2c24e7343293a75a22612a6fb5475501b7622999205b16539e8bf31c6ba464"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf7, 0xca, 0xb3, 0xda }; // Iskies: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IskiesMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IskiesMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IskiesMiner(CWallet *pwallet)
{
    printf("IskiesMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("iskies-miner");
    ...
    try { loop {
        ...
        printf("Running IskiesMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IskiesMiner, pwallet));
}
